...
//Function to prepare data for the chart
    function prepareDataForTable($fetchedData) {
        $labels = [];
        $datasets = [];
        $meanDeviationData = [];
        $meanArray = [];
        ...
        foreach ($fetchedData as $data) {
            ...
            if ($datasetIndex === false) {
                ...
                $meanArray[] = $data['mean_score'];
                print_r($data['CourseName']);
                print_r(' Mean Values :');
                print_r($data['mean_score']);
                $meanDeviationData = $data['mean_score'];
                print_r(' value1: ');
                print_r($meanArray[0]);
                print_r(' value2: ');
                print_r($meanArray[1]);
                $meanDeviation = $meanArray[1]-$meanArray[0];
                print_r(' Test for mean :');
                print_r($meanDeviation);
        ...
        foreach ($meanArray as $mean) {
            if(!empty($mean[$x])  && !empty($mean[$x-1])){
                print_r('first :'.$mean[$x].' second :'.$mean[$x+1]);
                $meanDeviation = $mean[$x]-$mean[$x-1];
                print_r(' After changes The New Mean Deviation ');
                print_r($meanDeviation);
            }
            $x=$x+1;
        }
    ...
    if (empty($selectedCourses) || empty($selectedExamTypes) || empty($selectedFormGroups)) {
        ...
        $chartData = prepareDataForTable($results);
                if (empty($results)) {
